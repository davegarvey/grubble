...
- **`node`** — updates the `version` field in `package.json` and `package-lock.json` (including the root entry under `packages[""]`), leaving key order, indentation, line endings and all other fields unchanged. Pairs with `npm publish`.
