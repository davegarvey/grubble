...
fn test_push_to_branch() {
    ...
    // Add a feat commit to trigger a minor bump (v1.0.0 -> v1.1.0)
    Command::new("git")
    ...
    // Run grubble with --push --tag and --git-branch to push the bump to a release branch
    cmd.arg("--push");
    cmd.arg("--tag");
    cmd.arg("--git-branch");
    ...
    // Verify the v1.1.0 tag exists on the remote (pushed via --tags)
    ...
    // Verify main was NOT updated (commit should only be on the release branch)
...
fn test_push_to_branch_with_force_tags() {
    ...
    // Run grubble with --push --tag --update-minor-tag --git-branch to exercise
    // the push_with_force_tags path
    cmd.arg("--push");
    cmd.arg("--tag");
    ...
    // Verify the minor tag (v1.1) was force-pushed to the remote
    let ls_tags_output = Command::new("git")
        .args([
            "ls-remote",
            "--tags",
