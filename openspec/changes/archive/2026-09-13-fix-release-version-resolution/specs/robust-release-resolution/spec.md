...
When `GITHUB_TOKEN` is available, the action SHALL use authenticated GitHub API calls (via `gh api` or the token) to query release information and avoid unauthenticated rate limits. When no token is available, the action SHALL use its documented unauthenticated fallback and warn the user.
