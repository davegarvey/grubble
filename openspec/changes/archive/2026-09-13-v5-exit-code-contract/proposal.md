...
- Add `--output text|json` flag for bump, `--bump-type`, and `--raw` modes, providing machine-readable output for CI integrations
...
- `machine-readable-output`: Defines the `--output text|json` flag for bump, `--bump-type`, and `--raw` modes, providing stable JSON schemas for CI integrations and downstream tooling.
