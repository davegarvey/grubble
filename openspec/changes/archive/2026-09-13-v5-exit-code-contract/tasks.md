...
- [x] 3.5 Support `--output json` in normal bump and `--dry-run` paths; emit a payload after a bump and no payload for a successful no-op
...
- [x] 3.8 Add `tests/cli_test.rs::test_json_output_invalid_with_dry_run` asserting a successful no-op emits no payload
- [x] 3.9 Add `tests/cli_test.rs::test_json_output_normal_run_no_bump` asserting a successful no-op emits no payload
...
- [x] 6.7 Add a "Machine-readable output" section to `README.md` documenting the JSON schemas for `--bump-type` and `--raw`
