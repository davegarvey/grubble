...
Superseded by `fix-release-workflow-redesign`. The temporary-branch approach shipped in PR #118, then the workflow moved to `--release-version` in PR #122. The stale delta is retained for historical context and must not be promoted to the canonical spec.
