...
Defines the read-only CLI operation used to extract the latest CHANGELOG entry for release pull-request and GitHub Release descriptions.
...
#### Scenario: CHANGELOG.md exists with a single entry
- **WHEN** the user runs `grubble --changelog-entry` and `CHANGELOG.md` contains one version entry
- **THEN** the command SHALL print that complete entry and exit 0
...
The `--changelog-entry` flag SHALL appear in `grubble --help` output with a one-line description. The README SHALL list the flag in the CLI usage section.
