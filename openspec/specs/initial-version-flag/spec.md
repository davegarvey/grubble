...
Defines how grubble bootstraps version calculation in repositories that have no reachable release tag.
...
The `--changelog-entry` flag SHALL remain a pure read-only operation that reads CHANGELOG.md. Combining it with `--initial-version` SHALL result in a usage error because the operations are incompatible.
...
### Requirement: --initial-version works with the default git preset
When no preset is configured (the default git preset), `--initial-version` SHALL remain valid. The command SHALL use the git strategy for the current version and SHALL use the supplied initial version as the bump baseline when no tag exists.
