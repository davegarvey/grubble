# robust-release-resolution Specification

## Purpose

Defines reliable release lookup for composite-action execution, including floating references, authenticated API access, asset validation, and non-composite compatibility.

## Requirements

### Requirement: Resolve pinned action version in composite action
The action SHALL resolve the pinned version of grubble when consumed as a composite action from another repository. The resolution SHALL pass `github.action_ref` through the action environment so it is evaluated in the composite-action context.

#### Scenario: Pinned full semver tag resolves correctly
- **WHEN** a consumer workflow uses `uses: davegarvey/grubble@v5.3.0`
- **THEN** the action SHALL download the binary from the `v5.3.0` release

#### Scenario: Floating major tag resolves to latest matching release
- **WHEN** a consumer workflow uses `uses: davegarvey/grubble@v5`
- **THEN** the action SHALL download the binary from the latest v5.x.y release that has assets
...
- **THEN** the action SHALL download the binary from the latest v5.3.x release that has assets

### Requirement: Authenticated API calls for release resolution
When `GITHUB_TOKEN` is available, the action SHALL use authenticated GitHub API calls to query release information and avoid unauthenticated rate limits. When no token is available, the action SHALL use its documented unauthenticated fallback and warn the user.
...
- **THEN** the action SHALL proceed to download the matching platform binary
...
### Requirement: Graceful fallback on missing auth token
If `GITHUB_TOKEN` is not available in the runner environment, the action SHALL fall back to unauthenticated API access with a warning rather than failing solely because authentication is unavailable.
...
- **AND** the action SHALL NOT fail solely due to missing authentication

### Requirement: Not affect non-composite-action usage
The change to `action.yml` SHALL NOT affect behavior when the action is consumed in non-composite contexts, such as the self-build path in version.yml.
